var createExplorer = function(){
	var explorer=new Ext.create('Ext.container.Container',{     
	  ...
	  items:[{
	    ...
	    bbar:[{
	    		xtype:'button',text:'Add current catalog',iconCls:'icon-shoppingbasket',handler:function(){
	    			var identifiers = [];
	    			identifiers.push({id:catalogURL,catalogURL:catalogURL});
	    			postIdentifiersToBasket(identifiers);
	    		}
	    	},{xtype:'tbseparator'},
	    	{
	    		xtype:'button',text:'Add file(s)',iconCls:'icon-shoppingbasket',handler:function(){
					...
					if(dapURL.length<4)dapURL=undefined;
					if(httpURL.length<4)httpURL=undefined;
	    	...
	    	{xtype:'button',text:'View file header',iconCls:'icon-view',handler:function(){
	    		var tree=explorer.getComponent('catalog-listing-tree');
				var numSelectedColumns = tree.getSelectionModel().getCount();
				var selection = tree.getSelectionModel().getSelection();
				var skipMessage="";
				if(numSelectedColumns!=1){
					alert("Please select one file to view (currently "+numSelectedColumns+" files selected)");
				}
				var el = {};
	    		...
	    		if(dapURL==undefined||dapURL.length<4)dapURL=httpURL.replace("fileServer","dodsC");
	      ...
	      proxy: {
	    	  type: 'ajax',url: serviceURL+'service=catalogbrowser',
	    	  listeners:{
	    		  load:{
	      ...
	      root: {text: 'catalog',id: catalogURL,expanded: true},
	      folderSort: false,
	      ...
	      listeners:{
	    	beforeload:{
	    		fn:function(t,aa,records,successful){
	    			var t=Ext.getCmp('ext_container');if(t)t.setLoading(true); 
	    		}
	    	},
	        load:{
	          fn:function(t,aa,records,successful){
			...
			listeners:{ 
				itemdblclick:function(i,record,item,index,e,options){
					if(record.isLeaf()==true){
						...
						identifiers.push({id:id,url:url});
						postIdentifiersToBasket(identifiers);
					}
				},
				itemclick:function(i,record,item,index,e,options){
					if(record.isLeaf()==true){
						explorer.selectedItem=i.getStore().getAt(index).get('OPENDAP');
					}
				}
			},
		  loadDapURL:function(OpenDAPURL){
			  var t=Ext.getCmp('ext_container');if(t)t.setLoading(true); 
	      var html="Dataset <a href=\"/impactportal/data/datasetviewer.jsp?dataset="+OpenDAPURL+"\">"+OpenDAPURL+"</a><br/>";
	      ...
	      var getVarURL=serviceURL+'service=getvariables&request='+OpenDAPURL;
	      proxy.url=getVarURL;
