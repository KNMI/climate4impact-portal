...
var wpsProcessorDetails = function(record){
   var url = impactWPSURL+'service=processor&request=describeProcessor&id='+record.get('id');
...
var selectProcessor = function(){
	if(selectedProcessor==undefined){alert("Please select a processor");return;};
	window.location="wpsuseprocessor.jsp?processor="+selectedProcessor.get('id');
	//alert(selectedProcessor.get('id'));
}
var wpsOverView={       xtype:'container',
        layout:'border',
        height:500,
        items:[
        {
                ...
                title:'WPS Overview',
                 store:  new Ext.data.Store({
                        ...
                        fields: [{name: 'id',       type: 'string'},
                                     ...
                                     {name: 'abstract',  type: 'string'}],
                                            proxy: {
                                                ...
                                                reader: {type: 'json',root: 'processors'}
                                            }
                                          }),
                        columns: [
                                {text: "Identifier", flex: 1, dataIndex: 'id', sortable: true},
                                {text: "Name", flex: 1, dataIndex: 'name', sortable: true},
                                {text: "Abstract", flex: 2, dataIndex: 'abstract', sortable: true,renderer:columnWrap}
                        ...
                        buttons:[{xtype:'button',text:'use',handler:function(){selectProcessor();}}]
        }]
}
