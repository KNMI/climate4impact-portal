/*
 * WMJSImage
  * image._srcToLoad the source to load
...
var WMJSImage = function(src,callback,__type){
  ...
  _this.checkIfThisSourceIsSet = function(src){
     if(_srcLoaded == src || _srcToLoad == src){
       return true;
     }
     return false;
  };
   ...
   */
  _this.load = function(){
    //console.log("WMJSImage:load "+_srcToLoad);
    ...
    if(_srcToLoad == _srcLoaded){
      //console.log("==== Already loaded ==== :"+_srcToLoad);
      loadEvent(_this);
      return;
    }
    ...
    el.attr("src", _srcToLoad+"&"+Math.random());
  };
  ...
  var loadEvent= function(image,hasError){
    ...
    //console.log("WMJSImage:ready "+_srcToLoad);
    //console.log("WMJSImage:loadEvent");
    _hasError = hasError
    _isLoading=false;
    _isLoaded=true;
    _srcLoaded = _srcToLoad;
