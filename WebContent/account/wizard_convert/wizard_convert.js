...
//  console.log("Adding layer"+layer.name)
  ...
  var WMSReady = function(l,service){
    ...
    var foundLayerInService = false;
    if(activeLayer){
      if(activeLayer!=""){
        var activeService = WMJSgetServiceFromStore('/impactportal/ImpactService?source='+resource+'&');
        console.log("activeService="+activeService.service);
        console.log("service="+service.service);
        if(activeService.service == service.service){
          for(var j=0;j<l.length;j++){
            var n = l[j];
            if(n!='baselayer'&&n!='overlay'&&n!='grid10'){
              if(selectedLayer == n){
                foundLayerInService = true;
              }
            }
          }
        }
