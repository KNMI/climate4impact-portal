...
<%@ page language="java" contentType="text/html; charset=UTF-8" import="tools.* "%>
...
<c:set var="isSubscribed" value="false" scope="request"/>
 ...
 <%
 	try{
  		...
  		if(downscaling.DownscalingAuth.isUserSubscribed(user)){
 		...
 		out.print("<input type='text' name='username' value='"+user.getUserId()+"' hidden></input>");
   	...
   	}catch(HTTPTools.WebRequestBadStatusException e){			
   		out.print("<div class='warning'>");
   		out.print(e.getResult());
   		out.print("</div>");
   	}catch(Exception e){
   		out.print("<div class='warning'>");
