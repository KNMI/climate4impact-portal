...
rm -rf certificates && mkdir certificates

echo "Putting cert for accounts.google.com"
echo | openssl s_client -connect accounts.google.com:443 2>&1 | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p'  > ./certificates/accounts.google.com.pem
keytool -delete -alias accounts.google.com  -keystore esg-truststore.ts -storepass changeit > /dev/null
keytool -import -v -trustcacerts -alias accounts.google.com -file ./certificates/accounts.google.com.pem -keystore esg-truststore.ts -storepass changeit -noprompt

echo "Putting cert for slcs.ceda.ac.uk"
echo | openssl s_client -connect slcs.ceda.ac.uk:443 2>&1 | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p'  > ./certificates/slcs.ceda.ac.uk.pem
